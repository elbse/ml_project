...
print("=" * 65)
print("  Android Malware Detection — Full Pipeline")
print("=" * 65)
...
print(f"\n  NOTE: Malware is the MAJORITY class ({n_malware/len(y)*100:.1f}%)")
print(f"        scale_pos_weight = {n_malware/n_benign:.2f} (boosts benign/minority)")
...
for row in baseline_results:
    ...
    if palma_acc:
        ...
        sign = "+" if dev >= 0 else ""
        print(f"  {name:<35} {palma_acc:>10.2f} {our_acc:>10.2f} {sign+str(round(dev,2)):>10}")
...
print(f"\n  NOTE: Deviations expected — different dataset version/features")
...
xgb_smote = ImbPipeline([
    ...
    ("clf", XGBClassifier(
        ...
        random_state=RANDOM_STATE,
        n_jobs=-1,
        # scale_pos_weight not needed — SMOTE already balances classes
    )),
])
...
for col in ["Accuracy", "Precision", "Recall", "F1-Score"]:
    ...
    diff = v2 - v1
    sign = "+" if diff >= 0 else ""
    print(f"  {col:<16} {v1:>11.2f}% {v2:>11.2f}% {sign+str(round(diff,2))+'%':>10}")
...
# =============================================================================
#  5. FALSE NEGATIVE ANALYSIS (key for Objective 3)
# =============================================================================
...
# Check proposed vs best baseline FN
...
if fn_improvement > 0:
    ...
    print(f"  Increase of {abs(fn_improvement)} more missed — see F1 and AUC for context")
...
# Color list: baselines=blue, xgb variants=amber, proposed=purple
...
# 6b. F1-Score comparison (primary metric)
hbar_plot(
...
# 6d. False Negatives comparison (lower is better)
fig, ax = plt.subplots(figsize=(11, 6))
...
# 6f. XGBoost SMOTE impact (Obj 2 — side by side)
...
ax.bar(x - width/2, v1, width, label="XGBoost (no SMOTE)",   color=AMBER,  edgecolor="white")
ax.bar(x + width/2, v2, width, label="XGBoost (with SMOTE)", color=DPURP, edgecolor="white")
ax.set_xticks(x)
ax.set_xticklabels(cols, fontsize=10)
ax.set_ylim(0, 115)
ax.set_ylabel("Score (%)", fontsize=10)
ax.set_title("Objective 2: XGBoost — impact of SMOTE", fontsize=11, fontweight="bold")
ax.legend(fontsize=9)
...
# 6g. Proposed model — per-metric bar
fig, ax = plt.subplots(figsize=(7, 4))
...
print(f"""
  ...
  (Palma et al. reported   : 79.14% — deviations documented)
